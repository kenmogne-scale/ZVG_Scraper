...
function decimal(value, digits = 2) {
  if (typeof value !== "number") return "-";
  return new Intl.NumberFormat("de-DE", { minimumFractionDigits: 0, maximumFractionDigits: digits }).format(value);
}
...
function renderAnalysisNotes(item) {
  ...
  if (!notes.length) return '<p class="small">Noch keine automatische KPI-Analyse ausgefuehrt.</p>';
...
function renderSourceSummary(item) {
  ...
  if (!rows.length) return '<p class="small">Noch keine Quellenhinweise verfuegbar.</p>';
...
function renderMetrics() {
  ...
  const totalValue = pipeline.reduce((sum, i) => sum + (i.valuationAmountEur ?? 0), 0);
  metricValueEl.textContent = totalValue > 0 ? eur(totalValue) : "-";

  const totalBids = pipeline.reduce((sum, i) => sum + (i.pipeline?.targetBidEur ?? 0), 0);
...
function applyFilters() {
  ...
  const contextParts = [`${new Intl.NumberFormat("de-DE").format(pagination.totalItems)} favorisierte Deals`];
...
const excelTable = createExcelTable({
  table: "table",
  rows: rowsEl,
  storageKey: "favorites-excel-table",
  columns: [
  ...
  emptyRow: (colspan) => `<tr class="empty-row"><td colspan="${colspan}">Noch keine Favoriten. Markiere Deals in der Pipeline mit dem Stern.</td></tr>`,
  onRender: (items) => renderResultCount(items.length)
});

function renderTable(items) {
  excelTable.apply(items.length ? items : []);
}

function renderPagination() {
  paginationInfoEl.textContent = `Seite ${pagination.page} von ${pagination.totalPages} | ${new Intl.NumberFormat("de-DE").format(pagination.totalItems)} Favoriten`;
  ...
  detailContentEl.innerHTML = `
    <div class="detail-shell">
      <section class="detail-main">
        ...
        <div class="detail-grid-two">
          <div class="detail-section">
            <div class="section-headline"><div><span class="section-kicker">Objekt</span><h4>Beschreibung</h4></div></div>
            <p class="modal-description">${escapeHtml(item.description ?? "-")}</p>
          </div>
          <div class="detail-section">
            <div class="section-headline"><div><span class="section-kicker">Pruefung</span><h4>Quellen der KPI-Analyse</h4></div></div>
            ${renderSourceSummary(item)}
          </div>
        </div>

        <div class="detail-section">
          <div class="section-headline"><div><span class="section-kicker">Unterlagen</span><h4>Dokumente und Anhaenge</h4></div></div>
      ...
      <aside class="detail-sidebar">
        <div class="detail-section">
          <div class="section-headline"><div><span class="section-kicker">Decision</span><h4>Analyse und Bewertung</h4></div></div>
          ...
          <form class="form-grid" id="analysis-form">
            <div class="form-row form-row--stack">
              <label class="form-field"><span>Strategie-Fit</span><input name="strategyFit" value="${escapeHtml(analysis.strategyFit ?? "")}" /></label>
              <label class="form-field"><span>Decision</span>
            ...
            <label class="form-field"><span>Notizen</span><textarea name="notes" rows="5">${escapeHtml(analysis.notes ?? "")}</textarea></label>
            <button type="submit" class="button-primary">Analyse speichern</button>
          </form>
        </div>

        <details class="detail-section detail-section--collapsed">
          <summary><span><span class="section-kicker">Sekundaer</span><strong>Pipeline bearbeiten</strong></span><span class="summary-hint">Nur wenn noetig</span></summary>
          <form class="form-grid" id="pipeline-form">
            ...
            <div class="form-row form-row--stack">
              ...
              <label class="form-field"><span>Prioritaet</span>
            ...
            <label class="form-field"><span>Investment-These</span><textarea name="thesis" rows="2">${escapeHtml(ps.thesis ?? "")}</textarea></label>
            <label class="form-field"><span>Naechster Schritt</span><textarea name="nextStep" rows="2">${escapeHtml(ps.nextStep ?? "")}</textarea></label>
            <label class="form-field"><span>Zielgebot (EUR)</span><input name="targetBidEur" type="number" step="1000" min="0" value="${escapeHtml(ps.targetBidEur ?? "")}" /></label>
...
async function toggleFavorite(auctionKey, button) {
  ...
  try {
    ...
    if (!res.ok) throw new Error(payload.error ?? "Favorit konnte nicht gespeichert werden");

    const index = pipeline.findIndex((entry) => entry.auctionKey === auctionKey);
    if (index >= 0) pipeline[index] = payload;

    // If unfavorited, remove from list and re-render
    if (!nextValue) {
      pipeline = pipeline.filter((entry) => isFavoriteItem(entry));
      renderMetrics();
    }
  } catch (error) {
...
async function reload() {
  try {
    const [serverFavorites, pipelinePayload] = await Promise.all([
      fetch("/api/favorites").then((res) => res.ok ? res.json() : { items: [] }).catch(() => ({ items: [] })),
      fetch(`/api/pipeline?page=1&pageSize=500&scope=all`).then((res) => res.ok ? res.json() : { items: [] }).catch(() => ({ items: [] }))
    ]);

    const merged = new Map();
    for (const item of serverFavorites.items ?? serverFavorites) {
      if (item?.auctionKey) merged.set(item.auctionKey, item);
    }
    for (const item of pipelinePayload.items ?? []) {
      if (item?.auctionKey && isFavoriteItem(item)) merged.set(item.auctionKey, item);
    }
    pipeline = [...merged.values()];
    pagination = { page: 1, pageSize: TABLE_PAGE_SIZE, totalItems: pipeline.length, totalPages: 1, hasNextPage: false, hasPreviousPage: false };
    renderMetrics();
    renderPagination();
    applyFilters();
  } catch (error) {
    rowsEl.innerHTML = `<tr class="empty-row"><td colspan="9">${escapeHtml(error.message)}</td></tr>`;
  }
}
...
rowsEl.addEventListener("click", (event) => {
  ...
  if (favoriteButton) {
    ...
    toggleFavorite(favoriteButton.dataset.auctionKey, favoriteButton).catch(console.error);
    return;
  }
...
detailContentEl.addEventListener("click", async (event) => {
  ...
  try {
    const res = await fetch(`/api/analysis/${encodeURIComponent(auctionKey)}/run`, { method: "POST" });
    const payload = await res.json();
    if (!res.ok) throw new Error(payload.error ?? "Analyse fehlgeschlagen");
